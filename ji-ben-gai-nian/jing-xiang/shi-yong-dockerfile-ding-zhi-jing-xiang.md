...
Dockerfile 是一个文本文件，其内包含了多条的 **指令\(Instruction\)**，**每一条指令构建一层**，因此每一条指令的内容，就是描述该层应当如何构建。
...
{% hint style="danger" %}
Dockerfile 中每一个指令都会**建立一层,**`RUN` 也不例外。也就是说每一个命令都会启动一个容器、执行命令。
{% endhint %}
...
Dockerfile 支持 Shell 类的行尾添加 `\` 的命令换行方式，以及行首 `#` 进行注释的格式。良好的格式，比如换行、缩进、注释等，会让维护、排障更为容易，这是一个比较好的习惯。因此镜像构建时，一定要确保每一层只添加真正需要添加的东西，任何无关的东西都应该清理掉。
...
当我们进行镜像构建的时候，并非所有定制都会通过 `RUN` 指令完成，经常会需要将一些本地文件复制进镜像，比如通过 `COPY` 指令、`ADD` 指令等。而 `docker build` 命令构建镜像，其实并非在本地构建，而是在服务端，也就是 Docker 引擎中构建的。在这种客户端/服务端的架构中，**为了让服务端获得本地文件，引入了上下文的概念。**
...
因此，`COPY` 这类指令中的源文件的路径都是_**相对路径**_。这也是初学者经常会问的为什么 `COPY ../package.json /app` 或者 `COPY /opt/xxxx /app` 无法工作的原因，因为这些路径已经超出了上下文的范围，Docker 引擎无法获得这些位置的文件。如果真的需要那些文件，应该将它们复制到上下文目录中去。
...
* `docker build` 命令后的路径是上下文路径，不是dockerfile 的路径。上下文路径主要方便**服务端获得本地文件。**
* dockerfile 中 `COPY`, `ADD` 后的路径一定是相对路径。
* docker build 命令默认执行将上下文目录下的名为 `Dockerfile` 的文件作为 Dockerfile。
